...
public class PayPalSucess {
	...
	public PayPalResult getPayPal(HttpServletRequest request, PayPalConfig payPalConfig) {
		...
		try {
			...
			req.setDoInput(true);
			req.setRequestProperty("Content-Type", "application/x-www-form-urle");
			...
			if (res.equals("SUCCESS")) {
				while ((res = in.readLine()) != null ) {
					...
					if (temp[0].equals("mc_hangling2")) {
						payPalResult.setMc_hangling2(temp[1]);
					}
					...
					if (temp[0].equals("patment_fee")) {
						payPalResult.setPatment_fee(temp[1]);
					}
					...
					if (temp[0].equals("reansaction_subject")) {
						payPalResult.setReansaction_subject(temp[1]);
					}
