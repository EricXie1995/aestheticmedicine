						...
						<ul>
							...
							<li class="submenu">
								...
								<ul style="display: none;">
									...
									<li><a href="<c:url value='/adminQuestion' />">貼文管理</a></li>
									<li class="active"><a href="<c:url value='/adminType' />">分類管理</a></li>
								</ul>
							</li>
							<li class="menu-title"> 
								<span>Pages</span>
...
<!-- 								</ul> -->
					...
					<div class="row">
						<div class="top-nav-search"> 
							<form action="<c:url value="/adminType/typeSearch" />">
							<c:if test="${likeString == null || likeString == ''}">
								<input type="text" name="likeString" class="form-control" placeholder="搜尋">
							</c:if>
							<c:if test="${likeString != null && likeString != ''}">
								<input type="text" name="likeString" value="${likeString}" class="form-control" placeholder="搜尋">
							</c:if>
								<button class="btn" type="submit"><i class="fa fa-search"></i></button>
							</form>
						</div>
						<div class="div1" style="text-align: right;"><a href="#Add_Specialities_details${type.typePkId}" data-toggle="modal" class="btn btn-primary float-right mt-2" style="color: white;text-align: right;">新增</a></div>
							<div class="modal fade" id="Add_Specialities_details${type.typePkId}" aria-hidden="true" role="dialog">
								<div class="modal-dialog modal-dialog-centered" role="document" >
									<div class="modal-content" style="text-align: left;">
										...
										<div class="modal-body">									
											<form action="<c:url value="/adminType/addType" />" method="post">
												<div class="row form-row">
													<div class="col-12 col-sm-12">
														<div class="form-group">
															<label>分類名稱</label>
															<input id="addType" name="typeName" type="text" class="form-control">
															<p class="form-control-static text-danger"><form:errors path="type.typeName"/></p>
														</div>
													</div>
												</div>
												<button id="addButton" type="submit" class="btn btn-primary btn-block">新增</button>
											</form>
										</div>
									</div>
								</div>
										...
										<table class="datatable table table-hover table-center mb-0">
											...
											<c:forEach var="type" items="${types}">
												<tr>
													...
													<td class="text-right">
														<div class="actions">
															<a data-toggle="modal" href="#edit_invoice_report${type.typePkId}" class="btn btn-sm bg-success-light mr-2">
																<i class="fe fe-pencil"></i> 編輯
															</a>
															<div class="modal fade" id="edit_invoice_report${type.typePkId}" aria-hidden="true" role="dialog">
																<div class="modal-dialog modal-dialog-centered" role="document" >
																	<div class="modal-content" style="text-align: left;">
																		<div class="modal-header">
																			...
																			<form action="<c:url value="/adminType/addType" />" method="post">
																				<div class="row form-row">
																					<div class="col-12 col-sm-12">
																						<div class="form-group">
																							<label>分類名稱</label>
																							<input type="hidden" name="typePkId" class="form-control" value="${type.typePkId}">
																							<input id="updType${type.typePkId}" name="typeName" type="text" class="form-control" value="${type.typeName}">
																							<p class="form-control-static text-danger"><form:errors path="type.typeName"/></p>
																						</div>
																					</div>
																				</div>
																				<button onclick="upd(this)" data-id="${type.typePkId}" id="updButton${type.typePkId}" type="submit" class="btn btn-primary btn-block">更新</button>
																			</form>
																		</div>
																	</div>
																</div>
															...
															<a class="btn btn-sm bg-danger-light" data-toggle="modal" href="#delete_modal${type.typePkId}">
																<i class="fe fe-trash"></i> 刪除
															</a>
															<div class="modal fade" id="delete_modal${type.typePkId}" aria-hidden="true" role="dialog">
															<div class="modal-dialog modal-dialog-centered" role="document" >
																...
																<!--	<div class="modal-header">
																	...
																	<div class="modal-body" style="text-align: center;">
																		<div class="form-content p-2">
																			...
																			<p class="mb-4">確定要<span style="color: red;">刪除${type.typeName}</span>嗎?</p>
																			<a href="<c:url value='/adminType/delType/${type.typePkId}' />"><button type="button" class="btn btn-primary">確定</button></a>
...
// 			分類新增，判斷為空無法送出
			$("#addButton").click(function(event){
					if($("#addType").val() == null || $("#addType").val() == "") {
						event.preventDefault();	
					}
				});
			
// 			分類更新，判斷為空無法送出未完成，研究中
			function upd(e) {
				var id = e.getAttribute("data-id");	
				$("#updButton"+id).click(function(event){
					if($("#updType"+id).val() == null || $("#updType"+id).val() == "") {
						event.preventDefault();	
					}
				});
			}
